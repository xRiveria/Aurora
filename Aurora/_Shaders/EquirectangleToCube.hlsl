...
Texture2D inputTexture : register(t1); // Cubemap texture loaded in from HLSL.
RWTexture2DArray<float4> outputTexture : register(u0); // Our cubemap that will be sampling from the input texture.

SamplerState defaultSampler : register(s1);

// Calculate normalized sampling direction based on current fragment coordinates. 
