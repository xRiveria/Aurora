...
bool ShadowCalculation(float4 fragPositionInLightSpace)
{
    ...
    projectionCoords = projectionCoords * 0.5 + 0.5; // As the depth from the depth map is in the range [0, 1], we transfrom our NDC to the range [0, 1] for sampling from the depth map.
    float closestDepth = Texture_ShadowDepthMap.Sample(objectSamplerState, projectionCoords.xy).r; // Gives us the closest depth from the light's POV.
    float currentDepth = projectionCoords.z; // Retrieve the current fragment's depth.
    bool isInShadow = closestDepth > currentDepth;
    
    if (isInShadow)
    {
        return true;
    }
    return false; // Check whether current depth is higher than closest depth. If so, this fragment is in shadow.
}
