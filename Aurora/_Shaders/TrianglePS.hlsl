...
bool ShadowCalculation(float4 fragPositionInLightSpace)
{
    float bias = 0.005;
    ...
    projectionCoords = projectionCoords * float3(0.5f, -0.5f, 1.0f) + float3(0.5f, 0.5f, 0.5f); // As the depth from the depth map is in the range [0, 1], we transfrom our NDC to the range [0, 1] for sampling from the depth map.
    float closestDepth = Texture_ShadowDepthMap.Sample(objectSamplerState2, projectionCoords.xy).r; // Gives us the closest depth from the light's POV.
    float currentDepth = projectionCoords.z; // Retrieve the current fragment's depth.
...
PS_Output main(vs_out input) : SV_TARGET // Pixel shader entry point which must return a float4 RGBA color value. 
{
    ...
    if (ShadowCalculation(input.outFragPosInLightSpace))
    {
        for (int i = 0; i < 4; i++)
        {
            albedoColor *= g_Light_Intensity[i] * 10.0;
    ...
    float3 finalColor = float3(0.3, 0.3, 0.3) * Lo; // Our ambient is currently a constant factor. For IBL, we will take this into account.
