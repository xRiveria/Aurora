...
namespace Aurora
{
    ...
    void DX11_GraphicsDevice::BindResources(Shader_Stage shaderStage, const RHI_GPU_Resource* const* resources, uint32_t slot, uint32_t count, RHI_CommandList commandList)
    {
        AURORA_ASSERT(count <= 16);
        ID3D11ShaderResourceView* shaderResourceViews[16];
        for (uint32_t i = 0; i < count; ++i)
        {
            shaderResourceViews[i] = resources[i] != nullptr && resources[i]->IsValid() ? ToInternal(resources[i])->m_ShaderResourceView.Get() : nullptr;
        }

        switch (shaderStage)
        {
        case Shader_Stage::Vertex_Shader:
            m_DeviceContextImmediate->VSGetShaderResources(slot, count, shaderResourceViews);
            break;
        case Shader_Stage::Pixel_Shader:
            m_DeviceContextImmediate->PSSetShaderResources(slot, count, shaderResourceViews);
            break;
        default:
            break;
        }
